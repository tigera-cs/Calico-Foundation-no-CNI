...
```
kubectl create -f training-lab-workbooks/foundation/1.1-install-calico-k8s-lab/lab_manifests/1-custom-resources.yaml
```
...
As an additional task, we will install calicoctl as binary in our bastion host. For this step we will need to match the Calico version (latest available) to our calicoctl client. Let's retrieve the release and store in a variable with the command below:

```
export CALICO_VERSION=$(kubectl exec -it $(kubectl get pod -n tigera-operator -o custom-columns='Name:.metadata.name' --no-headers) -n tigera-operator -- operator --version | grep Calico | cut -d ' ' -f2)
```

Then let's check the current version for our installation:
